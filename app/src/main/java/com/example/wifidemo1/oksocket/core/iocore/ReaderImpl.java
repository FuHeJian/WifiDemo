...
import com.example.wifidemo1.oksocket.core.exceptions.ReadException;
...
import com.example.wifidemo1.oksocket.core.utils.BytesUtils;
...
public class ReaderImpl extends AbsReader {
    ...
    public void initialize(InputStream inputStream, IStateSender stateSender) {
        super.initialize(inputStream, stateSender);
        isClose = false;
    }
    ...
    public void read() throws RuntimeException {
        if (isClose) {
            SLog.e("ReaderImpl read() isClose = true");//560
            return;
        }
        ...
        try {
            ...
            while (readCount < count) {
                len = mInputStream.read(readBytes, readCount, count - readCount);
                readCount += len;
            }
            originalData.setBodyBytes(readBytes);
